...
document.onload = loadPage();
	function loadPage() {
		var page = ${sessionScope.spage} > 0 ? ${sessionScope.spage}:<%=ReadXml.getKeys("limit2")%>;
		$.ajax({
			type : "POST",
			url : "./Store_ListAction.co",
			data : {
				"mk" : "true",
		...
		<c:choose>
			<c:when test="${listcount > 0 }">
				<table width=60% border="0" cellpadding="0" cellspacing="0">
					...
					<td align="right"><div id="adress_list"> <font size=2px>검색된 결과 수 : ${listcount}</font></div></td>
					</tr>
					<tr>
