...
<%
	...
	boolean cI = false;
	if(user != null){
		if(user.getINFO_INDEX()==bean.getSTORE_INDEX())
			cI = true;
	}
...
<script>
	...
	$(document).ready(function(){
		if((!${sessionScope.user.getINFO_POSITION().equals(ReadXml.getKeys("position_admin"))})&&(!${sessionScope.user.getINFO_POSITION().equals(ReadXml.getKeys("position_seller"))})){
			alt();
			location.href= "./WhatEat/Home/View_Main.jsp";
		}
	})
	function addStar(){
		$.ajax({
		...
		<div id="box2_wrap_det">
			...
			<div class="menu_wrap" style="display:none">
				...
				<div class="wrap_sidebar_det">
					...
					<div class="bookmark_det" style="cursor:pointer" onclick="javascript:location.href='./Seller_Store_ListAction.co'">
						<b>목록으로</b>
					</div><br/>
					...
					<% if(user != null){%>
					<%if((user.getINFO_MODSTORE_RG().equals("TRUE")&& cI) || cP){ %>
					<div class="bookmark_det" style="cursor:pointer" onclick="javascript:location.href='<%="."+ReadXml.getKeys("View_MyPagePath")+ReadXml.getKeys("View_SellerPath2")%>Store_Modify.jsp?index=<%=bean.getSTORE_NUM()%>'"> <b>수정하기</b> </div><br/>
					<%}else if((!user.getINFO_MODSTORE_RG().equals("TRUE")) || !cI){%>
					<div class="bookmark_det" style="cursor:pointer" onclick="alt()"> <b>수정하기</b> </div><br/>
					<%}if((user.getINFO_DELSTORE_RG().equals("TRUE")&&cI) || cP){ %>
					<div class="bookmark_det" style="cursor:pointer" onclick="javascript:{if(confirm('정말 삭제 하시겠습니까?'))location.href='./Store_DeleteAction.co?dnum=<%=bean.getSTORE_NUM()%>';}"> <b>삭제하기</b> </div><br/>
					<%}else if((!user.getINFO_DELSTORE_RG().equals("TRUE"))||!cI){%>
					<div class="bookmark_det" style="cursor:pointer" onclick="alt()"> <b>삭제하기</b> </div><br/>
					<%} }%>
